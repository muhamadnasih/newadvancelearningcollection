...
namespace ConsoleUI
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            List<IPizza> pizzacart = AddPizzaData();


            foreach (var item in pizzacart)
            {
                item.Preparation();
            }
        ...
        private static List<IPizza> AddPizzaData()
        {
            List<IPizza> pizzaorder = new List<IPizza>();

            pizzaorder.Add(new PapperoniPizza ());
            pizzaorder.Add(new MagarittaPizza());
            return pizzaorder;
        }
    }
}
